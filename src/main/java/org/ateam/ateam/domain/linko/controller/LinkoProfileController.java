...
public class LinkoProfileController {
  ...
  @GetMapping("/me")
  @Operation(summary = "내 프로필 조회", description = "등록한 회사 프로필 정보를 조회합니다.")
  ...
  public ResponseEntity<ResponseDto<LinkoProfileResDTO>> getMyLinko(
    ...
    return ResponseEntity.ok(ResponseDto.of(HttpStatus.OK, null, "프로필 조회 성공", profile));
  }

  @GetMapping
  @Operation(summary = "회사 페이징 목록 조회", description = "등록된 회사 프로필 목록을 페이징하여 조회합니다.")
  public ResponseEntity<ResponseDto<PagedResponse<LinkoProfileResDTO>>> getPage(Pageable pageable) {
    ...
    return ResponseEntity.ok(ResponseDto.of(HttpStatus.OK, null, "Linko 목록 조회 성공", response));
  }

  @GetMapping("/{linkoId}")
  @Operation(summary = "특정 회사 프로필 조회", description = "linkoId로 특정 회사 프로필을 조회합니다.")
  @ApiErrorCodeExamples({ErrorCode.ENTITY_NOT_FOUND})
  public ResponseEntity<ResponseDto<LinkoProfileResDTO>> getLinkoById(@PathVariable Long linkoId) {
  ...
  public ResponseEntity<ResponseDto<Void>> deleteLinko(
      @PathVariable Long linkoId, @AuthenticationPrincipal UserAuthentication auth) {
