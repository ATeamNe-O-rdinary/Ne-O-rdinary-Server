...
public interface LinkerRepository extends JpaRepository<Linker, Long> {
  ...
  @Query(
      value =
          "SELECT l "
              + "FROM Linker l "
              + "LEFT JOIN Link k ON k.linker = l "
              + // Link의 linker 필드와 조인
      ...
      countQuery =
          "SELECT COUNT(l) FROM Linker l WHERE l.jobCategory IN :categories") // 페이징을 위한 카운트 쿼리 별도
  // 지정
