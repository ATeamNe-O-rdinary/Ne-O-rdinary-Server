...
public class LinkerController {
  ...
  public ResponseDto<LinkerResponse> create(@RequestBody LinkerCreateRequest request) {
    ...
    return ResponseDto.of(HttpStatus.CREATED, "LINKER_CREATE_SUCCESS", "링커 정보 생성 성공", response);
  }

  @Operation(summary = "링커 단건 조회")
  @ApiErrorCodeExamples({ErrorCode.LINKER_NOT_FOUND})
  @GetMapping("/{linkerId}")
  public ResponseDto<LinkerResponse> getById(@PathVariable Long linkerId) {
    ...
    return ResponseDto.of(HttpStatus.OK, "LINKER_GET_SUCCESS", "링커 조회 성공", response);
  }
  ...
  public ResponseDto<PagedResponse<LinkerResponse>> getPage(Pageable pageable) {
    ...
    return ResponseDto.of(HttpStatus.OK, "LINKER_PAGE_SUCCESS", "링커 목록 조회 성공", response);
  }
  ...
  public ResponseDto<LinkerResponse> update(
      @PathVariable Long linkerId, @RequestBody LinkerUpdateRequest request) {

    Long memberId = userContext.getCurrentUserId();

    LinkerResponse response = linkerService.linkerUpdate(memberId, linkerId, request);

    return ResponseDto.of(HttpStatus.OK, "LINKER_UPDATE_SUCCESS", "링커 정보 수정 성공", response);
  }
  ...
  public ResponseDto<Void> delete(@PathVariable Long linkerId) {
    ...
    return ResponseDto.of(HttpStatus.NO_CONTENT, "LINKER_DELETE_SUCCESS", "링커 삭제 성공");
  }
}
